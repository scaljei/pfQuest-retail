...
local function CreateBrowseWindow(fname, name, parent, anchor, x, y)
  ...
  parent.tabs[fname].button:SetScript("OnClick", function(self, button)
...
pfBrowser.close = CreateFrame("Button", "pfQuestBrowserClose", pfBrowser)
pfBrowser.close:SetPoint("TOPRIGHT", -5, -5)
pfBrowser.close:SetHeight(20)_em_placeholder
...
pfBrowser.close:SetScript("OnClick", function(self, button)
  self:GetParent():Hide()
end)
...
pfBrowser.journal = CreateFrame("Button", "pfQuestJournalOpen", pfBrowser)
pfBrowser.journal:SetPoint("TOPRIGHT", -30, -5)
pfBrowser.journal:SetHeight(20)_em_placeholder
...
pfBrowser.journal:SetScript("OnClick", function(self, button)
...
pfBrowser.clean:SetScript("OnClick", function(self, button)
...
pfBrowser.input.searchIcon = pfBrowser.input:CreateTexture(nil, "OVERLAY")
pfBrowser.input.searchIcon:SetTexture(pfQuestConfig.path.."\\img\\tracker_search")
pfBrowser.input.searchIcon:SetHeight(14)_em_placeholder
...
pfBrowser.input.clearButton:SetScript("OnClick", function(self, button)
...
pfBrowser.input:SetScript("OnTextChanged", function(self)
  ...
  for _, caption in ipairs({"Units","Objects","Items","Quests"}) do
    local searchType = string.lower(caption)

    local data = (string.len(text) >= 3 or custom) and pfDatabase:GetIDByName(text, searchType, true, custom) or pfBrowser_fav[searchType]
